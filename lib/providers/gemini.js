...
async function generateAnalysis(clientReq, apiKey, model, systemInstruction, customerText, temperature) {
  ...
  const factor = 2;
  const maxDelay = 8000;
  const overallDeadline = 90000;
  const perAttemptTimeout = 30000;
  ...
  while (attempt < maxAttempts) {
    ...
    if (clientReq && (clientReq.aborted || (clientReq.socket && clientReq.socket.destroyed))) {
    ...
    try {
      ...
      console.log(`[Debug] Gemini API returned status: ${status}, body: ${result.body}`);
      ...
      if (err.code === 'aborted' || (clientReq && (clientReq.aborted || (clientReq.socket && clientReq.socket.destroyed)))) {
